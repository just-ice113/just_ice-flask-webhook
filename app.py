...
# Match these exactly with what you typed into your Render dashboard!
...
# --- Simple user storage (Highly recommend replacing with a database later) ---
user_data = {}
...
def add_referral(inviter_id, new_user_id):
    ...
    # Safely initialize the new user only if they don't already exist
...
def telegram_webhook():
    ...
    # Guard clause against non-message updates from Telegram
    ...
    if text.lower().startswith("refer"):
        ...
        # Fix: Strip out the word "write" so it doesn't mess up your AI generation
        prompt = text[5:].strip()
        if not prompt:
    ...
    try:
        ...
        if resp.status_code != 200:
            # Fallback to plain text if the AI returns messy Markdown/HTML formatting that breaks Telegram
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage",
