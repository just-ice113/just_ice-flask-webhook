...
def generate_ai_content(prompt):
    ...
    if response.status_code == 200:
        ...
        # Format nicely for Telegram (Markdown)
        formatted = f"**Blog Post: {prompt.title()}**\n\n{content}"
        return formatted
    else:
...
def telegram_webhook():
    ...
    requests.post(
        ...
        json={"chat_id": chat_id, "text": reply, "parse_mode": "MarkdownV2"}  # ✅ formatted output
    )
    return "ok", 200
...
def test_openrouter():
    ...
    return response.text  # show full OpenRouter response (success or error)
