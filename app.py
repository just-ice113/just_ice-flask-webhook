...
def generate_ai_content(prompt):
    ...
    payload = {
        # FIXED: Using the automatic free router to avoid 404 model errors
...
def add_referral(inviter_id, new_user_id):
    ...
    # FIXED: Safely initialize the new user only if they don't already exist
...
def telegram_webhook():
    ...
    # FIXED: Guard clause against non-message updates from Telegram
    ...
    if text.lower().startswith("refer"):
        ...
        # FIXED: Strip out the word "write" so it doesn't mess up your AI generation
        prompt = text[5:].strip()
        if not prompt:
    ...
    try:
        ...
        if resp.status_code != 200:
            # Fallback to plain text if HTML styling breaks Telegram parsing
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage",
